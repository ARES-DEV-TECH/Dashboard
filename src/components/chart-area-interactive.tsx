  ...
  return (
    <Card>
      ...
      <CardContent className="px-2 pt-4 sm:px-6 sm:pt-6">
        ...
        >
          {chartType === 'area' ? (
          <AreaChart key={`area-${data?.length ?? 0}-${areaMountKey}`} data={data} margin={{ top: 8, right: 8, bottom: 8, left: 8 }}>
            <defs>
            ...
            {canStackArea && (
              <>
                <Area dataKey="charges" type="natural" fill="url(#fillCharges)" stroke="var(--color-charges)" stackId="a" isAnimationActive animationDuration={800} animationEasing="ease-out" />
                <Area dataKey="result" type="natural" fill="url(#fillResult)" stroke="var(--color-result)" stackId="a" isAnimationActive animationDuration={800} animationEasing="ease-out" />
              </>
            )}
            {showCa && !canStackArea && (
              <Area dataKey="ca" type="natural" fill="url(#fillCa)" stroke="var(--color-ca)" isAnimationActive animationDuration={800} animationEasing="ease-out" />
            )}
            {showCa && canStackArea && (
              <Line dataKey="ca" type="monotone" stroke="var(--color-ca)" strokeWidth={2} dot={false} name="CA HT" isAnimationActive animationDuration={800} animationEasing="ease-out" />
            )}
            {showCharges && !canStackArea && (
              <Area dataKey="charges" type="natural" fill="url(#fillCharges)" stroke="var(--color-charges)" isAnimationActive animationDuration={800} animationEasing="ease-out" />
            )}
            {showResult && !canStackArea && (
              <Area dataKey="result" type="natural" fill="url(#fillResult)" stroke="var(--color-result)" isAnimationActive animationDuration={800} animationEasing="ease-out" />
            )}
            <ChartLegend
